/**
 * A nearest neighbor algorithm module:
...
#define NN_POPULATION MAX_POPULATION // this is the max population for the nearest neighbor code.
	// so, each node will construct NN_POPULATION tours.
...
void populate_tours(int N, int mpi_rank, tour_t** arr_tours, tour_t* arr_cities) {
	...
	while (i<NN_POPULATION) {
		...
		arr_tours[i] = create_tour_nn(arr_cities->city[city_offset%N], N, arr_cities);
...
/*
	...
	Tours = malloc( sizeof(tour_t*) * NN_POPULATION );
...
*/
	...
	if (mpi_rank==0) {
		...
		for (i=1;i<mpi_procs;i++) {
			MPI_Recv(tempIntTour, CitiesA->size*3, MPI_INT, i, MPI_TAG, MPI_COMM_WORLD, &status);
		...
		MPI_Send(tempIntTour, CitiesA->size*3, MPI_INT, 0, MPI_TAG, MPI_COMM_WORLD);
	}
